...
export function AboutOrbCanvas({
  ...
  React.useEffect(() => {
    ...
    const resize = () => {
      ...
      dpr = Math.min(window.devicePixelRatio || 1, useCompact ? 1.5 : 2)
    ...
    const draw = (now: number) => {
      ...
      // Responsive liquid: snappier attack, soft release.
      ...
      const dotBase = compact ? 0.36 : isImmersive ? 0.52 : 0.44
      ...
      const spinSpeed = 0.14 + smoothed * 0.22

      context.clearRect(0, 0, width, height)

      // Futuristic energy field — scales with voice.
      {
      ...
      // Voice ring — thin futuristic halo.
      {
      ...
      context.filter = compact ? "blur(0.5px)" : "blur(0.4px)"
